...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.cap="重复测量数据方差分析流程图", fig.align="center", out.width="80%", fig.height=4}
library(diagram)
...
textrect(elpos[7, ], radx = radx + 0.05, rady = rady, shadow.size = 0, lab = "多重比较\n单独效应分析等", cex = cex)
bias <- 0.03
cex <- 0.7
...
由于存在5个处理分组，每个分组进行了4次重复测量，故共有20组数据需要做正态性检验。考虑将数据转换成长数据方便使用`lapply`函数一次性完成所有分组的正态性检验。
...
DT::datatable(long_dat)
```

使用Shapiro-Wilk检验进行正态性检验。该检验零假设为样本来自于正态分布总体，故p值小于0.05时考虑拒绝原假设，即样本来自于非正态总体。
...
使用Levene检验进行方差齐性检验。该检验零假设为样本所在总体方差相等，故p值小于0.05时考虑拒绝原假设，即样本所在总体方差不等。
...
```
结果与上方使用base R进行方差分析和球形检验的结果基本一致，在最后多出了用Greenhouse-Geisser和Huynh-Feldt方法进行校正后的p值。
