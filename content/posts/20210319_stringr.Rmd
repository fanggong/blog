...
对应地，base R中也有相对应的函数达到同样的效果。在我的本地环境下，发现base R中字符串操作函数的性能在数据较大情况下普遍不如[stringr](https://mirrors.tuna.tsinghua.edu.cn/CRAN/web/packages/stringr/index.html)。
...
我在Mac、Windows、Centos下分别对[stringr](https://mirrors.tuna.tsinghua.edu.cn/CRAN/web/packages/stringr/index.html)和base R的性能进行了测试，发现在不同的环境，不同的数据集下有很大差距。基本可以概括为在使用小数据集时，不同的环境下的结果没有什么规律可循，在使用大数据集时，[stringr](https://mirrors.tuna.tsinghua.edu.cn/CRAN/web/packages/stringr/index.html)略胜一筹。
