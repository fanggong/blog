...
- **pad**：是否在图的左右两侧加空白，若为"TRUE"，最后呈现的图从高度为0的柱开始到高度为0的柱结束
...
### `geom_path()`
```{r}
fyc <- tibble(long = c(1,5,4,3,2), lat = c(1,1,2,3,2))
ggplot(fyc, aes(long, lat)) + geom_path()
```

### `geom_line()`
```{r}
ggplot(fyc, aes(long, lat)) + geom_line()
```

### `geom_step()`
```{r}
ggplot(fyc, aes(long, lat)) + geom_step(direction = "hv")
```
